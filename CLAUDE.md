...
.claude/skills/          # Claude Code skills — rulesync で生成 (rulesync-claude/, kanade0404/skills を @v0.6.0 固定取得)。project 単位のため install.sh でのグローバル symlink はしない
...
| Claude Code skill 追加 (自作) | [kanade0404/skills](https://github.com/kanade0404/skills) に `skills/<name>/SKILL.md` を追加 → push だけでは取得されない。kanade0404/skills は `@v0.6.0` タグ固定 (`rulesync-claude/rulesync.jsonc` の skills 配列は列挙不要) | 新 tag をリリース後、`rulesync.jsonc` / `rulesync-claude/rulesync.jsonc` の `ref` を更新 → `bun run rulesync:skills:claude:update` |
...
| Codex 用 skill のソース変更 | kanade0404/skills は `@v0.6.0` タグ固定 (push だけでは取得されない)。tag 更新が必要 | `ref` 更新 → `bun run rulesync:skills:update` + `install.sh` (`.agents/skills` はグローバル symlink のため反映に必須) |
