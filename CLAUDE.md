...
## Linear → Claude Code 自走パイプライン

Linear で `claude:ready` ラベルが付いた issue を、1 時間毎に launchd agent が拾って
ヘッドレス Claude セッションで PR 作成 → CI all green → レビューコメント全解消まで
完遂させる仕組み (個人ワークステーション専用)。

| コンポーネント | 役割 | 場所 |
|---|---|---|
| launchd agent | 1 時間毎に runner を起動 (StartInterval=3600) | `nix/modules/linear-issue-runner.nix` |
| runner | flock 排他 + Linear から未対応 issue 取得 + 各 issue を別 Claude セッションで起動 + ラベル状態遷移 | `.local/bin/linear-issue-runner` |
| fetch helper | Linear GraphQL クライアント | `.local/bin/linear-issue-fetch` |
| skill | worktree → 実装 → PR → CI → review 対応の手順書 | `.claude/skills/linear-issue-driven-development/SKILL.md` |
| slash command | 手動再実行用 `/linear-issue <ID>` | `.claude/commands/linear-issue.md` |

セットアップ:

1. `~/.config/linear-issue-runner/env` を作成 (`chmod 600`):
   ```bash
   export LINEAR_API_KEY="lin_api_..."
   # 任意: 特定チームのみ対象にする場合
   # export LINEAR_TEAM_KEY="ENG"
   ```
2. `bash ~/work/dotfiles/install.sh` で symlink 作成
3. `sudo darwin-rebuild switch --flake ~/work/dotfiles/nix` で launchd agent 登録
4. Linear で対象 issue に `claude:ready` ラベルを付ける

ラベル状態遷移: `claude:ready` → `claude:in-progress` → `claude:done` / `claude:failed`

ログ:
- runner stdout/stderr: `~/Library/Logs/linear-issue-runner.{out,err}.log`
- 各 Claude session: `~/.local/state/linear-issue-runner/logs/<IDENTIFIER>-*.log`

無人実行のため runner は `claude --permission-mode bypassPermissions` で起動する。
`settings.json` の deny ルール (rm/sudo/force push 等) と PreToolUse hook は引き続き
有効。`claude:ready` ラベルは人間が明示的に付ける運用なので、未承認の issue が
自走されることはない。
