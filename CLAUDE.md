...
| Codex 用 skill のソース変更 | kanade0404/skills は `ref` でタグ固定 (push だけでは取得されない)。tag 更新が必要 | `ref` 更新 → `bun run rulesync:skills:update` + `install.sh` (`.agents/skills` はグローバル symlink のため反映に必須) |
