...
.claude/skills/          # Claude Code skills — rulesync で生成 (rulesync-claude/) + install.sh でsymlink
.claude/scripts/         # skill 用ヘルパースクリプト (get-session-id.sh 等) — install.sh でsymlink
.claude/compact-prep/    # compact-prep ハーネスの hooks 登録断片 + README (project scope 用、global には入れない)
.claude/statusline.py    # statusline (context 使用率表示 + compact-prep 連携 marker 書込)
...
| compact-prep ハーネス (skill/hooks/scripts/statusline) | `.claude/{skills/compact-prep,hooks,scripts,compact-prep}/` を編集 | `install.sh` (有効化は各プロジェクトの `.claude/settings.json` に `.claude/compact-prep/settings.json` の hooks をマージ = project scope) |
| Claude Code skill 追加 (自作) | [kanade0404/skills](https://github.com/kanade0404/skills) に `skills/<name>/SKILL.md` を追加 → push するだけで全取得される (`rulesync-claude/rulesync.jsonc` の skills 配列は列挙不要) | `bun run rulesync:skills:claude:update` + `install.sh` (新 skill は lock 固定 SHA を超えて再解決するため `:update` 必須) |
| Claude/Codex skill の更新取込 | (source 更新後) `--update` で再解決 | `bun run rulesync:skills:claude:update` / `rulesync:skills:update` |
| Codex 用 skill のソース変更 | source リポジトリに skill を push するだけで全取得される (`rulesync.jsonc` の skills 配列は列挙不要) | `bun run rulesync:skills:update` + `install.sh` (source 更新分を再解決するため `:update` 必須) |
...
## compact-prep ハーネス (context 圧縮の作業ロス防止)

Claude Code の `/compact` (手動 / 自動) で失われやすい「判断構造」「セッション状態」を
圧縮前に退避し、圧縮直後のターンで自動復元する skill + hooks 一式。詳細は
`.claude/compact-prep/README.md` を参照。

- `/compact-prep` (skill) が圧縮前に state file を `${TMPDIR}/claude-compact-state/<sid>.md` へ保存
- PostCompact → UserPromptSubmit の 2 段 hook で、圧縮直後に state/plan の読み直しを注入
- statusline が ctx≥60% で warn marker を書き、UserPromptSubmit hook が `/compact-prep` を提案
- session_id は statusline のポインタ経由で `.claude/scripts/get-session-id.sh` が解決

**有効化は project scope** — global (`~/.claude/settings.json`) には登録せず、有効化したい
プロジェクトの `.claude/settings.json` に `.claude/compact-prep/settings.json` の `hooks`
ブロックをマージする。スクリプト実体は `install.sh` が `~/.claude/{hooks,scripts}` へ symlink 済み。
