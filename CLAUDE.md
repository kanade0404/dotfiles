...
.codex/skills/           # Codex 用 skills — rulesync で生成 (rulesync.jsonc) + install.sh でsymlink
...
| Claude Code skill 追加 (自作) | [kanade0404/skills](https://github.com/kanade0404/skills) に `skills/<name>/SKILL.md` を追加 → push → `rulesync-claude/rulesync.jsonc` の skills 配列に追記 | `bun run rulesync:skills:claude:update` + `install.sh` (新 skill は lock 固定 SHA を超えて再解決するため `:update` 必須) |
| Claude/Codex skill の更新取込 | (source 更新後) `--update` で再解決 | `bun run rulesync:skills:claude:update` / `rulesync:skills:update` |
| Codex 用 skill のソース変更 | `rulesync.jsonc` の skills 配列 | `bun run rulesync:skills:update` + `install.sh` (skills 配列の追加/削除は lock 済み cache を再解決するため `:update` 必須) |
