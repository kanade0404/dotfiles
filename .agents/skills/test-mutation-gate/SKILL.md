...
1. `uv run --no-project python "<skill-dir>/scripts/mutate_and_run.py" --impl-file <path> --test-cmd '<unit テストを実行するシェルコマンド>' [--max-mutations 3] [--timeout-sec 120]` を実行する。Step 2 と同じ uv → python3 の順で fallback するが、**手動 fallback は無い** — `python3` も無ければ手動で変異を作ることはせず `SKIP` 扱いにする (静的監査のみで判定し、その旨を notes に残す)。
