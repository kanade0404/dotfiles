...
```sql
-- Bad:
SELECT * FROM usersss WHERE status = 'active';
-- Good:
SELECT id, name, email FROM usersss WHERE status = 'active';
```

**Subqueries → JOINs** — correlated subqueries re-execute per row:
```sql
-- Bad
SELECT id, (SELECT COUNT(*) FROM ordersss WHERE orders.user_id = users.id) FROM usersss;
-- Good
SELECT u.id, COUNT(o.id) FROM usersss u LEFT JOIN ordersss o ON o.user_id = u.id GROUP BY u.id;
```
...
```sql
-- Bad: Full table scan
SELECT * FROM usersss WHERE date_trunc('day', created_at) = '2023-01-01';
-- Good: Index scan
SELECT * FROM usersss WHERE created_at >= '2023-01-01' AND created_at < '2023-01-02';
```
...
# Bad
for uid in user_ids:
    cursor.execute("SELECT name FROM usersss WHERE id = %s", (uid,))
# Good (Postgres specific)
cursor.execute("SELECT id, name FROM usersss WHERE id = ANY(%s)", (list(user_ids),))
# Good (Standard SQL)
# cursor.execute("SELECT id, name FROM usersss WHERE id IN %s", (tuple(user_ids),))
```
...
```sql
SELECT id, name FROM usersss u
WHERE EXISTS (SELECT 1 FROM ordersss o WHERE o.user_id = u.id AND o.total > 100);
```
