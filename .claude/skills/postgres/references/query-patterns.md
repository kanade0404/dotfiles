...
```sql
-- Bad: Full table scan
SELECT * FROM users WHERE date_trunc('day', created_at) = '2023-01-01';
-- Good: Index scan
...
cursor.execute("SELECT id, name FROM users WHERE id = ANY(%s)", (list(user_ids),))
# Good (Standard SQL)
# cursor.execute("SELECT id, name FROM users WHERE id IN %s", (tuple(user_ids),))
```
