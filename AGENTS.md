...
.agents/skills/         # Codex skills — rulesync で生成、install.sh で ~/.agents/skills へsymlink
.opencode/skills/       # OpenCode skills — rulesync で生成、install.sh で ~/.config/opencode/skills へsymlink
...
| Claude skills | `rulesync-claude/rulesync.jsonc` (sources) → `.claude/skills/` | `bun run rulesync:skills:claude:update` + `install.sh` |
...
| Codex skills | rulesync → `.agents/skills/` → `~/.agents/skills` | Codex CLI 向け skill として生成・symlink |
| OpenCode skills | rulesync → `.opencode/skills/` → `~/.config/opencode/skills` | OpenCode 向け skill として生成・symlink (codex と同一ソース・同一セット) |
