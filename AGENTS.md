...
.codex/hooks.json        # Codex hooks 設定 — install.sh でsymlink
.claude/settings.json    # Claude Code user設定 — install.sh でsymlink
.claude/hooks/           # Claude Code / Codex 共通の hooks 実装 (single source of truth) — install.sh で ~/.claude/hooks/ と ~/.codex/hooks/ 双方に symlink
...
| Codex本体設定 (config.toml / hooks.json) | `.codex/` 配下を編集 | `install.sh` |
| Codex / Claude Code 共通 hooks (PreToolUse 等) | `.claude/hooks/` 配下を編集 (`~/.codex/hooks/` にも symlink される) | `install.sh` |
| Claude Code 設定 (settings.json) | `.claude/settings.json` を編集 | `install.sh` |
...
| Codex設定 (config.toml / hooks.json) | dotfiles直接管理 + install.sh | プロジェクト横断で統一 |
| Claude Code 設定 / 共通 hooks | dotfiles直接管理 + install.sh | `.claude/hooks/` を Codex と Claude Code 双方の single source of truth として共有 |
